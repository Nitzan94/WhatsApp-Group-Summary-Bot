...
class SchedulerService {
  ...
  async initialize() {
    try {
      logger.info('🕒 מאתחל מערכת תזמונים מבוססת מסד נתונים...');
      ...
      logger.info(`✅ מערכת תזמונים מבוססת DB הופעלה - ${this.dbScheduledTasks.length} משימות פעילות`);
  ...
  async createDBTaskCronJob(task) {
    try {
      ...
      const job = cron.schedule(cron_expression, async () => {
        // CRITICAL DEBUG: First log to see if cron callback is triggered
        logger.info(`🔥 CRON TRIGGERED! משימה ${id} (${name}) - ${new Date().toLocaleString('he-IL', { timeZone: 'Asia/Jerusalem' })}`);
